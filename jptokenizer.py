...
class Cleaner(object):
    def __init__(self):
        self.url = re.compile(pattern='((www\.[^\s]+)|(https?://[^\s]+))')
        self.mention = re.compile(pattern='@[^\s]+')
        self.hash_tag = re.compile(pattern=r'#([^\s]+)')
        self.white_space = re.compile(pattern='[\s]+')

    def process_tweet(self, tweet):

        #Converte www.* or https?://* to URL
        tweet = re.sub(pattern=self.url, repl='URL', string=tweet)
        #Converte @username to AT_USER
        tweet = re.sub(pattern=self.url, repl='AT_USER', string=tweet)
        #Remove additional white space
        tweet = re.sub(pattern=self.url, repl=' ', string=tweet)
        #Replace #word with word
        tweet = re.sub(pattern=self.url, repl=r'\1', string=tweet)
        #trim
        tweet = tweet.strip('\'"')
...
class JPSentTokenizer(RegexpTokenizer):
    def __init__(self):
        super(JPSentTokenizer, self).__init__(u'[^「」!?。]*[!?。]')
...
class JPMeCabTokenizer(TokenizerI):
    ...
    def tokenize(self, text):
        text = text['text']
        text = self.cleaner.process_tweet(tweet=text)
        result = self.mecab.parse(text)
        return result.strip().split(' ')

class JPSimpleTokenizer(TokenizerI):
...
if __name__ == '__main__':
    sentTokenizer = JPMeCabTokenizer()

    text = {'text':"私は大学生です。"}
    print(sentTokenizer.tokenize(text))
    cleaner = Cleaner()